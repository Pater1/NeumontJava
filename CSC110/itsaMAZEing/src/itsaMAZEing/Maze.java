...
public class Maze {
	...
	public static void main(String[] args){
		maze = generateMaze(Vector2.newVector(101,101), Vector2.newVector(1, 1), Vector2.newVector(2, 2));
		drawMaze(maze);
	}
	
	private static MazeSpace[][] generateMaze(Vector2 size, Vector2 start, Vector2 jump){
		return generateMaze(blankMaze(size),start,start,jump, 0);
	}
		...
		for(int i = 0; i < m.length; i += m.length-1){
			for(int j = 0; j < m[i].length; j++){
				if(m[i + ((i==0)?jump.y:-jump.y)][j] == MazeSpace.space){
					...
					doorDirs.add(Vector2.newVector(((i==0)?jump.y:-jump.y),0));
				}
			}
		}
		for(int i = 0; i < m[0].length; i += m[0].length-1){
			for(int j = 0; j < m.length; j++){
				if(m[j][i + ((i==0)?jump.x:-jump.x)] == MazeSpace.space){
					...
					doorDirs.add(Vector2.newVector(0,((i==0)?jump.x:-jump.x)));
				}
			}
		}
